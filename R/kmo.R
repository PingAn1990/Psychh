#' KMO (reported vs computed)
kmo <- function(data, varname, spec2 = spec2(),
  ...
  if (!is.na(key)) {
    ...
    if (!is.null(r)) {
      reported <- list(KMO = r$KMO, items = spec2$schema$items_by_construct[[key]],
                       source=list(table="table2", field="KMO"))
      matched <- TRUE
    }
  }
    ...
    if (length(items) >= 2) {
      ...
      try({
        if (use == "complete.obs") {
          ...
          n_used <- nrow(X)  # approximate; pairwise uses varying n
          R <- stats::cor(X, use="pairwise.complete.obs")
          diag$warnings <- c(diag$warnings, "pairwise.complete.obs: n_used is approximate; KMO may be unstable under missingness.")
        }
        km <- .kmo_from_cor(R)
        computed <- list(KMO = km$KMO, MSAi = km$MSAi, items = items, n_used = n_used, method=list(use=use))
        diag$n_used <- n_used
      }, silent=TRUE)
      if (is.null(computed)) diag$errors <- c(diag$errors, "Failed to compute KMO (possibly singular correlation matrix).")
    } else {
      diag$errors <- c(diag$errors, "Too few matched item columns to compute KMO.")
    }
  }
  ...
  if (!is.null(reported) && !is.null(computed)) {
    diff <- list(KMO_abs = abs(reported$KMO - computed$KMO))
  }

  .psychh_result("kmo", spec2, key = key, matched = matched, match = match, call = call,
